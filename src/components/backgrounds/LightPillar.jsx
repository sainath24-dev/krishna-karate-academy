...
export function LightPillar({
  ...
  useEffect(() => {
    ...
    // Optimized settings for smooth 60fps rendering without GPU starvation
    const qualitySettings = {
      low: { iterations: 18, waveIterations: 1, pixelRatio: 0.5, precision: 'mediump', stepMultiplier: 1.6 },
      medium: { iterations: 28, waveIterations: 1, pixelRatio: 0.75, precision: 'mediump', stepMultiplier: 1.3 },
      high: {
        iterations: 36,
        waveIterations: 2,
    ...
    // Pause rendering when element is offscreen to save GPU
    ...
    const handleMouseMove = (event) => {
      if (!interactive || !isVisibleRef.current) return;
  ...
  if (!webGLSupported) {
    return (
      <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode }}>
        <div className="pillar-static-glow" />
      </div>
    );
  }
